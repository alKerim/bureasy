...
export default function DashboardPage() {
  ...
  const generateChecklist = async (query) => {
    ...
    try {
      ...
      // **Key Change:** Directly treat `data` as the checklist object
      if (typeof data === "object" && data !== null) {
        // Validate the structure
        if (
          Array.isArray(data.steps) &&
          typeof data.source === "string" &&
          Array.isArray(data.pdf_links)){
          setParsedChecklist(data);
          setRawChecklist(JSON.stringify(data, null, 2));
          setCheckedSteps(new Array(data.steps.length).fill(false));
        } else {
          throw new Error("Invalid checklist structure.");
        }
      } else if (typeof data === "string") {
        // Fallback in case `data` is a JSON string
        ...
        try {
          ...
          if (
            ...
            Array.isArray(parsed.pdf_links) ) {
            setParsedChecklist(parsed);
            setCheckedSteps(new Array(parsed.steps.length).fill(false));
          } else {
        ...
        throw new Error("Unexpected checklist data format.");
      }
    } catch (err) {
          ...
          <div className="bg-white dark:bg-gray-700 p-6 rounded-lg shadow flex flex-col">
            ...
            {parsedChecklist ? (
              <div className="flex-1 max-h-80 overflow-y-auto" ref={checklistContainerRef}>
                ...
                {/* Source and PDF Links Section */}
                <div className="mt-6 space-y-4">
                  {parsedChecklist.source && (
                    <div className="bg-gray-100 dark:bg-gray-600 p-4 rounded-lg">
                      <h3 className="font-semibold mb-2">Source:</h3>
                      <a
                        href={parsedChecklist.source}
                        target="_blank"
                  ...
                  {parsedChecklist.pdf_links && parsedChecklist.pdf_links.length > 0 && (
                    <div className="bg-gray-100 dark:bg-gray-600 p-4 rounded-lg">
                      <h3 className="font-semibold mb-2">Relevant Documents:</h3>
                      <ul className="list-disc pl-5 space-y-2">
                        {parsedChecklist.pdf_links.map((link, index) => (
                          <li key={index}>
                            <a
                              href={link}
                              target="_blank"
              ...
              <div className="mt-6 bg-blue-50 dark:bg-blue-700 p-4 rounded-lg shadow flex flex-col">
                <div className="flex items-center flex-wrap">
                  ...
                  <a href={`tel:${humanPhoneRaw}`}>
                    ...
                    >
                      {/* **Recommendation:** Replace with a Phone Icon */}
                      {/* <ChatBubbleLeftRightIcon className="w-5 h-5 mr-2" /> */}
                      {/* Using PhoneIcon instead */}
                      <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a2 2 0 011.992 1.81L10 6.5a16.038 16.038 0 006 6l1.69-1.69a2 2 0 011.81-1.992L19 12h2a2 2 0 012 2v4a2 2 0 01-2 2h-3.28a2 2 0 01-1.992-1.81L15 18.5a16.038 16.038 0 00-6-6l-1.69 1.69a2 2 0 01-1.81 1.992L5 16H3a2 2 0 01-2-2v-4a2 2 0 012-2z" />
                      </svg>
                      Call Now
                    </button>
                  </a>
