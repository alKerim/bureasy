...
export default function AssistantResponsePage() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // If the flow is complete, use the assistant's response to generate the checklist
      if (data.finished) {
        setRequestText(data.response);
        generateChecklist(data.response);
      }
    } catch (err) {
      console.error(err);
  ...
  const handleSkip = async () => {
    ...
    try {
      ...
      generateChecklist(data.user_request); // Automatically generate checklist
    } catch (err) {
      console.error(err);
